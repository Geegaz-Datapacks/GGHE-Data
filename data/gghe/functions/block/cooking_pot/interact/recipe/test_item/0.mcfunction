...
data modify storage geegaz:gghe temp.ItemB set from storage geegaz:gghe temp.SimplifiedItems[0]
...
execute as @s[scores={gghe.var=0}] store result storage geegaz:gghe temp.SimplifiedItems[0].Count byte -1.0 run data get storage geegaz:gghe temp.SimplifiedItems[0].Count -1.0001
